...
def ot_loss(proto_s, feat_tu_w, feat_tu_s, pl):
    ...
    # score_sorted, sorted_ind = torch.sort(score_ot, descending=True)
    # feature matching loss
    # if (pred_ot == pl).float().sum() > 0:
    #     feat_tu_s = feat_tu_s[pred_ot == pl]
    #     pred_ot = pred_ot[pred_ot == pl]
...
def train(source_loader, target_loader, target_loader_unl, target_loader_val, target_loader_test,
    ...
    net_G_ema2 = ModelEMA(net_G, decay=0.999)
    net_F_ema2 = ModelEMA(net_F, decay=0.999)
    ...
    for batch_idx in range(args.num_steps):
        ...
        # mcc loss
        # logits_tu_w = no_grad_F(net_F, feat_tu_w, temp=args.T)
        # logits_tu_s = no_grad_F(net_F, feat_tu_s, temp=args.T)
        prob_tu_w = torch.softmax(logits_tu_w, dim=1)
        prob_tu_s = torch.softmax(logits_tu_s, dim=1)
        L_mcc = cross_mcc(prob_tu_s, prob_tu_w)
        ...
        # backward
        # Lx = (1 - args.lambda_b) * Lx_s + args.lambda_b * Lx_t
        # Lt = L_fix * args.lambda_u + L_mcc * args.lambda_mcc + (L_ent_1 + L_ent_2) * args.lambda_entmax
        # Loss = 2 * (Lx + Lt * args.lambda_b) + L_scatter * args.lambda_scatter
        Loss = Lx + L_scatter * args.lambda_scatter + L_fix * args.lambda_u \
            + L_mcc * args.lambda_mcc + (L_ent_1 + L_ent_2) * args.lambda_mcc * 0.1  \
            + lambda_warm * L_ot * args.lambda_ot + L_mme
        ...
        loss_value_list = [Lx.item(), L_fix.item(), consis_mask.sum().item() / consis_mask.shape[0],
                           L_mcc.item(), L_scatter.item(), L_align.item(), L_mme.item()]
